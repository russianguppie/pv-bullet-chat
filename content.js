(() => {
  ...
  chrome.extension.onMessage.addListener(function(msg, sender, sendRes) {
    ...
    switch(msg.type) {
      case "active":
        ...
        $iframe = $('<iframe>').prop('src', src + '?url=' + url).prop('id', 'bullet-frame')
          .css({border: 0, position: 'fixed', height: '120px', width: '240px', right: '-240px', bottom: '5%', 'z-index': 20000}).appendTo('body');
  ...
  function shoot(bullet) {
    let $bullet = $('<span>').prop('class', 'bullet').text(sanitize(bullet['content']))
      .css({position: 'fixed', top: bullet['position'], color: bullet['color'],
      'font-size': `${bullet['size']}rem`, opacity: opacity, 'white-space': 'nowrap', 'z-index': 10000}).appendTo('body');
    $bullet.css({'right': -Math.floor(1.25*$bullet.width())+'px'});
    $bullet.animate({right: '100%'}, bullet['duration'], 'linear', function() {
      $(this).remove();
    });
  }
})();
